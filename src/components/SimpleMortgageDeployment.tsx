...
export const SimpleMortgageDeployment = () => {
  ...
  const deployContract = async () => {
    ...
    try {
      toast({
        ...
        description: "Deploying SimpleMortgage contract to Fuji testnet...",
      });

      // Call the Supabase edge function to deploy the contract
      const { data, error } = await supabase.functions.invoke('deploy-simple-mortgage');
      ...
      if (data.success) {
        ...
        toast({
          ...
          description: `SimpleMortgage deployed to ${data.contractAddress}`,
        });
      } else {
    ...
    <Card className="max-w-2xl mx-auto">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2">
            ...
            SimpleMortgage Contract Deployment
          </CardTitle>
          <Badge variant="outline">Fuji Testnet</Badge>
        </div>
        <p className="text-muted-foreground">
          Deploy a clean, production-ready mortgage contract with real blockchain execution
        </p>
      </CardHeader>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="bg-muted/50 p-4 rounded-lg">
          <h3 className="font-medium mb-3">📋 Contract Features</h3>
          <div className="grid gap-2 text-sm">
            <div className="flex items-center gap-2">
              <CheckCircle className="w-4 h-4 text-green-600" />
              <span>Real property purchases with down payments</span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle className="w-4 h-4 text-green-600" />
              <span>Monthly mortgage payments with amortization</span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle className="w-4 h-4 text-green-600" />
              <span>Interest calculation and principal tracking</span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle className="w-4 h-4 text-green-600" />
              <span>Automated payment overdue detection</span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle className="w-4 h-4 text-green-600" />
        ...
        <div className="text-center space-y-4">
          ...
          >
            {isDeploying ? (
              ...
              <>
                🚀 Deploy SimpleMortgage Contract
              </>
            )}
          </Button>
        ...
        <div className="pt-4 border-t text-xs text-muted-foreground space-y-1">
          ...
          <div><strong>USDT Contract:</strong> 0x43eCed1b7C1BDc6522Db5a2F39905Cc0E3CE7F28</div>
