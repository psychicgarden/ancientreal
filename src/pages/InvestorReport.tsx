...
export default function InvestorReport() {
  ...
  useEffect(() => {
    document.title = "Investor Report | Ancient";
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) metaDesc.setAttribute("content", "Investor report with verified metrics: volume, reliability, utilization.");
    else {
      const m = document.createElement("meta");
      m.name = "description";
      m.content = "Investor report with verified metrics: volume, reliability, utilization.";
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        console.error(e);
        setError(e.message || "Failed to load investor report.");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="min-h-screen">
        <Header />
        <main className="container mx-auto px-6 lg:px-8 py-28">
          <SectionHeader title="Investor Report" subtitle="Loading verified metrics..." align="left" />
        </main>
        <Footer />
      </div>
    );
  ...
  if (error) {
    return (
      <div className="min-h-screen">
        <Header />
        <main className="container mx-auto px-6 lg:px-8 py-28">
          <SectionHeader title="Investor Report" subtitle={error} align="left" />
        </main>
        <Footer />
      </div>
    );
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="container mx-auto px-6 lg:px-8 py-28">
        <header>
          <h1 className="sr-only">Investor Report - Trust, Transparency and Performance</h1>
          <SectionHeader
            title="Investor Report"
            subtitle="Trust, transparency, and performance — verified from on-chain actions and real-time database metrics."
            align="left"
          />
        </header>
