-- Fix mortgage payment calculation in backfill function
...
-- Recalculate existing mortgage payments using correct formula
UPDATE public.user_properties 
SET monthly_payment = CASE 
  WHEN loan_amount_base > 0 THEN 
    -- Convert base units to USD, apply correct formula, convert back
    ((loan_amount_base / 1000000.0) * (0.08/12.0 / (1 - power(1 + 0.08/12.0, -120))))
  ELSE 0 
END,
updated_at = now()
WHERE loan_amount_base > 0;