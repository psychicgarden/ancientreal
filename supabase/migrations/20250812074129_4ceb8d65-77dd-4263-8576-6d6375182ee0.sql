-- Phase 1: Ensure idempotent setup for unique key and trigger, then backfill recent txs
...
-- 3) Optional: add a helpful index for wallet lookups in user_transactions (safe no-op if exists)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 4) Replay recent transactions to ensure user_properties is populated (function skips non-completed)
WITH recent AS (
  SELECT id
