...
`;
    ...
    fs.chmodSync(path.join(backupDir, 'restore-backup.js'), '755');
    ...
    const summary = {
      ...
      restoreCommand: `node ${path.join(backupDir, 'restore-backup.js')}`,
      nextSteps: [
...
\`\`\`bash
node ${path.join(backupDir, 'restore-backup.js')}
\`\`\`
